...
| [framework/platform/snailjob](../framework/platform/snailjob/) | API 内嵌 SnailJob 执行器 |
| [modules/](../modules/) | 业务包目录（同属根 module） |
| [modules/iam/account](../modules/iam/account/) | 业务包分层样板 |
| [app/](../app/) | 装配根：唯一运行入口 `cmd/api` + migrate；`modules/all` |
| [migrations/](../migrations/) | goose SQL（仓根） |
| [scripts/migrate.sh](../scripts/migrate.sh) | 迁移 |
| [script/docker/README.md](../script/docker/README.md) | 本地 SnailJob Server |
| [web/](../web/) | 前端 |
...
| 路径 | 职责 |
|------|------|
| `hei-gin/framework/...` | 配置、安全、中间件、Module 注册表、DB/Redis/存储、stringly/bind、SnailJob |
| `hei-gin/modules/...` | 业务；同包 `param`/`result`/`repo`/`service`/`handler`；`init` 自注册 |
| `hei-gin/modules/shared` | 跨业务共享 |
| `hei-gin/app/...` | 装配 + cmd；`internal/modules/all` 汇总 blank import |

## 约定摘要

- **单体**：一个 `go.mod`，仓根 `go run ./app/cmd/api` 直接启动。
