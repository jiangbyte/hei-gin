...
func (s *Service) registerRoutes(d *shared.Deps) module.RouteRegistrar {
	return func(api *gin.RouterGroup) {
		...
		admin.GET("/page", middleware.RequirePermission(d.Perms, "message:notice:page", "Notice page"), s.pageAdmin)
		admin.POST("/publish", middleware.RequirePermission(d.Perms, "message:notice:publish", "Publish notice"), s.publish)
		admin.POST("/revoke", middleware.RequirePermission(d.Perms, "message:notice:revoke", "Revoke notice"), s.revoke)
		admin.POST("/pin", middleware.RequirePermission(d.Perms, "message:notice:pin", "Pin notice"), s.pin)
		s.registerMyRoutes(admin)

		portal := api.Group("/v1/portal/message/notices")
...
func (s *Service) portalList(c *gin.Context) {
	var q PageParam
	_ = c.ShouldBindQuery(&q)
	rows, total, current, size, err := s.PagePublished(c.Request.Context(), q)
	if err != nil {
		response.Fail(c, http.StatusBadRequest, 400, err.Error())
		return
	}
...
func (s *Service) myDetail(c *gin.Context) {
	...
	row, err := s.MyDetail(c.Request.Context(), q.ID)
	if err != nil {
		response.Fail(c, http.StatusNotFound, 404, "notice not found")
		return
	}
