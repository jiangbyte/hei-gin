// internal/modules/biz/cg_test_catalog/handler.go HTTP 处理器。
//
// Author: Charlie
...
func (s *Service) registerRoutes(d *module.Deps) func(*gin.RouterGroup) {
	return func(api *gin.RouterGroup) {
		...
		g.POST("/delete", middleware.RequirePermission(d.Perms, "biz:cgtestcatalog:delete", "Delete catalog"), s.delete)
		g.GET("/detail", middleware.RequirePermission(d.Perms, "biz:cgtestcatalog:detail", "Catalog detail"), s.detail)
		g.GET("/page", middleware.RequirePermission(d.Perms, "biz:cgtestcatalog:page", "Catalog page"), s.page)
		g.GET("/tree", middleware.RequirePermission(d.Perms, "biz:cgtestcatalog:tree", "Catalog tree"), s.tree)
	}
}
