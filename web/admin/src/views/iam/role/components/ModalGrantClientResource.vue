...
const modalTitle = computed(() =>
  state.subject?.name
    ? `${state.title || '分配客户端资源'} - ${state.subject.name}`
    : state.title || '分配客户端资源',
)
...
async function fetchGrant() {
  ...
  state.loading = true
  try {
    const response = state.lockAccountType
      ? await state.grantApi.ownClientResources(state.subject.id)
      : await state.grantApi.ownClientResources(state.subject.id, state.accountType)
...
async function submitGrant() {
  ...
  try {
    ...
    if (!state.lockAccountType) {
      payload.account_type = state.accountType
    }
    await state.grantApi.grantClientResources(payload)
