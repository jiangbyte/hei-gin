...
foreach ($f in $files) {
  ...
  foreach ($ln in $arr) { $lines.Add($ln) }

  $pkgIdx = -1
  for ($i = 0; $i -lt $lines.Count; $i++) {
    if ($lines[$i] -match '^package\s+\w+') { $pkgIdx = $i; break }
  }
  if ($pkgIdx -lt 0) { Write-Output "skip-no-pkg $rel"; continue }

  # 从 package 行往上收集紧邻注释行
  $hdrs = New-Object System.Collections.Generic.List[string]
  $i = $pkgIdx - 1
  while ($i -ge 0) {
    $t = $lines[$i].Trim()
    if ($t -eq '') { break }
    if ($t -match '^//' -or $t -match '^/\*' -or $t -match '^\*/' -or $t -match '^\*') {
      $hdrs.Insert(0, $lines[$i])
      $i--
    } else { break }
  }
  $headerText = ($hdrs -join $nl)
  if ($headerText.Contains('Author: Charlie')) { $skipped++; continue }

  $base = [System.IO.Path]::GetFileNameWithoutExtension($f.Name)
  $desc = if ($descMap.ContainsKey($base)) { $descMap[$base] } else { '实现' }

  if ($hdrs.Count -gt 0 -and $hdrs[0].Trim() -match '^// Package ') {
    # package doc：在 doc 块末尾（package 前）追加 Author
    $lines.Insert($pkgIdx, '// Author: Charlie')
    $lines.Insert($pkgIdx, '//')
  } else {
    # 无 doc：插入文件头注释（含描述与作者）
    $head = @("// $rel $desc。", '//', '// Author: Charlie', '')
    for ($j = $head.Count - 1; $j -ge 0; $j--) { $lines.Insert(0, $head[$j]) }
  }
